...
#include "eorm_sqlite_rows_handler.hpp"
...
public:
    ...
    bool countRows(
        const std::function<void(size_t)> rowsCountHandler,
        const SqlExpr& sqlExpr) const
    {
        return sqliteDatabase.executeQueryAsync(
            Table::getSqlRowsCount(sqlExpr),
            std::make_shared<SqliteRowsCountHandler>(rowsCountHandler)
        );
    }

    bool selectRows(
        const std::function<void(size_t)> rowHandler,
        ...
        const std::function<void(size_t)> rowsCountHandler = nullptr) const
    {
        ...
        return sqliteDatabase.executeQueryAsync(
            std::move(sql),
            rowHandler ? std::make_shared<SqliteRowsHandler>( std::initializer_list<Table>{*this}, rowHandler ) : nullptr
        );
    }
